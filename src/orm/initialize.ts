...
export default async function initialize(): Promise<void> {
    ...
    let carol = new User('carol', 'password', 'Carol');
    let mike = new User('mike', 'qwerty', 'Mike');
    let alice = new User('alice', '123456', 'Alice');
    let sam = new User('sam', 'iloveyou', 'Sam');
    let greg = new User('greg', 'bravo', 'Greg');
    let peter = new User('peter', 'volcano', 'Peter');
    let bobby = new User('bobby', 'racecar', 'Bobby');
    let marcia = new User('marcia', 'davyjones', 'Marcia');
    let jan = new User('jan', 'glass', 'Jan');
    let cindy = new User('cindy', 'thindy', 'Cindy');
